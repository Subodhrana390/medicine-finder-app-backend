...
import { initEventHandlers } from "./src/events/eventHandlers.js";
...
initEventHandlers().catch(console.error);
...
app.use((req, res) => {
  res.status(404).json({
    success: false,
    message: `Route ${req.originalUrl} not found in medical shop service`
  });
});

app.listen(PORT, async () => {
  console.log(`🏥 Medical Shop Service running on port ${PORT}`);

  // Publish service started event
  try {
    await publishEvent(TOPICS.SYSTEM_EVENTS, EVENT_TYPES.SERVICE_STARTED, {
      serviceName: 'medical-shop-service',
      version: '1.0.0',
